...
static bool file_exists(const std::string& f) { std::ifstream in(f); return in.good(); }
static size_t file_count_lines(const std::string& f) { std::ifstream in(f); size_t n=0; std::string s; while(std::getline(in,s)) ++n; return n; }
...
TEST_CASE("LoggingTrajMPI: GatherRoot dump rank-as-type; global coords within box", "[MPI][log][dump]") {
    ...
    {
        LoggingTrajMPI logger(base, LoggingTrajMPI::Mode::GatherRoot, MPI_COMM_WORLD, /*append=*/false, /*rank_as_type=*/true);
    ...
    if (rank == 0) {
        ...
        while (std::getline(in, line)) {
            ...
            REQUIRE( (bool)(iss >> id >> type >> x >> y >> z) );
            REQUIRE(type >= 1);
            REQUIRE(type <= size);
            type_seen[type-1] += 1;
            REQUIRE(x >= 0.0 - 1e-9);
...
TEST_CASE("LoggingTrajMPI: PerRank xyz & dump basic write", "[MPI][log][perrank]") {
    ...
    {
        LoggingTrajMPI logger(base, LoggingTrajMPI::Mode::PerRank, MPI_COMM_WORLD, /*append=*/false, /*rank_as_type=*/true);
...
TEST_CASE("LoggingTrajMPI: MPI-IO dump rank-as-type; structure valid", "[MPI][log][mpiio][dump]") {
    ...
    // if (rank == 0) mkdir("unit_test_mpi_out", 0777);
    ...
    MPI_Barrier(MPI_COMM_WORLD); // make sure file is closed before reading

    // Single file written by MPI-IO; check on rank 0
    if (rank == 0) {
        const std::string fname = base + ".dump";
        REQUIRE(file_exists(fname));
