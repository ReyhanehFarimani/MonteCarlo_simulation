 ...
 */
Simulation::Simulation(const SimulationBox &box, PotentialType potentialType, SimulationType simtype, double temperature, int numParticles, double maxDisplacement, double r2cut, float f_prime,float alpha, float f_d_prime, float kappa, double mu, unsigned int seed, bool useCellList, int cellListUpdateFrequency)
    : box(box), simtype(simtype), potentialType(potentialType), temperature(temperature), numParticles(numParticles), maxDisplacement(maxDisplacement), rcut(r2cut), r2cut(r2cut * r2cut), f_prime(f_prime), alpha(alpha), f_d_prime(f_d_prime), kappa(kappa), mu(exp(mu/temperature)), energy(0.0), seed(seed), useCellList(useCellList), cellListUpdateFrequency(cellListUpdateFrequency) , numCellsX(1), numCellsY(1){
                ...
                for (int offsetY = -2; offsetY <= 2; ++offsetY) {
                    ...
                    for (int offsetX = -2; offsetX <= 2; ++offsetX) {
                        ...
                        for (CellListNode* node = cellList[neighborCellIndex]; node != nullptr; node = node->next) {
                            ...
                            if (r2 < r2cut) {
                                dE += computePairPotential(r2, potentialType, f_prime, f_d_prime, kappa, alpha); 
                            }
                        }
                    }
                }
    ...
    for (size_t i = 0; i < particles.size(); ++i) {
        for (size_t j = i + 1; j < particles.size(); ++j) {
            ...
            if (r2 < r2cut) {
                ...
                double potential = computePairPotential(r2, potentialType, f_prime, f_d_prime, kappa, alpha);
        ...
        for (int offsetX = -2; offsetX <= 2; ++offsetX) {
            ...
            for (CellListNode* node = cellList[neighborCellIndex]; node != nullptr; node = node->next) {
                if (node->particleIndex != particleIndex) {
                    ...
                    if (r2 < r2cut) {
                        localEnergy += computePairPotential(r2, potentialType, f_prime, f_d_prime, kappa, alpha);
                    }
                }
            }
        }
...
double Simulation::computeTotalForce() const{
    ...
    for (size_t p1 = 0; p1 < particles.size(); ++p1){
        for (size_t p2 = p1 + 1; p2 < particles.size(); ++p2){
            ...
            if (r2<r2cut){
                forceSum += computePairForce(r2, potentialType, f_prime, f_d_prime, kappa, alpha);
            }
        }
    }
    return forceSum;
